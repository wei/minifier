...
var _serviceWorkerHelper = _interopRequireDefault(require("./service-worker-helper"));

var _minifyWorkerHelper = _interopRequireDefault(require("./minify-worker-helper"));

var _helper = require("./helper");

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

_serviceWorkerHelper.default.load();

_minifyWorkerHelper.default.load();
...
var codeWrapperContainer = document.getElementById('code-wrapper-container');
...
var inputCM = _codemirror.default.fromTextArea(document.getElementById('input-code'), {
  lineNumbers: true,
  theme: 'neo',
  autofocus: true,
  dragDrop: true,
...
var outputCM = _codemirror.default.fromTextArea(document.getElementById('output-code'), {
  lineNumbers: true,
  lineWrapping: true,
  theme: 'neo',
  readOnly: true
...
var runMinify = (0, _helper.debounce)(function (type) {
  var input = inputCM.getDoc().getValue();
  ...
  if (type) {
    ...
    if (input.trim() === '') {
      ...
      _minifyWorkerHelper.default.minify(type, input, {}, function (output) {
...
var selectLanguage = function selectLanguage() {
  ...
  outputFileName.placeholder = "".concat(droppedFileName || 'edit-me').concat(type ? ".".concat(type) : '');
...
['js', 'css', 'html'].forEach(function (lang) {
  selectionButtons[lang].addEventListener('click', function (e) {
    e.preventDefault();
    selectLanguage(lang);
    inputCM.focus();
  });
  minifyButtons[lang].addEventListener('click', function (e) {
    e.preventDefault();
    selectLanguage(lang);
  });
});
...
document.getElementById('download').addEventListener('click', function (e) {
  ...
  _fileSaver.default.saveAs(new File([outputCM.getDoc().getValue()], outputFileName.value || outputFileName.placeholder, {
    type: 'text/plain;charset=utf-8'
  }));
});
inputCM.on('changes', function (doc) {
  if (!doc.getValue()) {
    outputCM.getDoc().setValue('');
    droppedFileName = null;
    selectLanguage('');
  ...
  if (currentType) {
    ...
    // Detect Type
    var detectedType = function (text) {
...
inputCM.on('paste', function () {
  currentType = null;
  droppedFileName = null;
});
inputCM.on('drop', function (cm, dragEvent) {
  try {
    var fileName = dragEvent.dataTransfer.files[0].name;
    var matches = fileName.match(/\.(js|css|html)$/);

    if (matches && matches[1]) {
      droppedFileName = fileName.replace(".".concat(matches[1]), '');
  ...
  } catch (_) {// Ignore fetch filename errors
  }
}); // Check Local Storage on first load

try {
  var inputText = window.localStorage.getItem('minifier.input');
  var type = window.localStorage.getItem('minifier.type');
...
},{"./helper":9,"./minify-worker-helper":11,"./service-worker-helper":12,"clipboard/dist/clipboard.min":2,"codemirror":3,"codemirror/mode/css/css":4,"codemirror/mode/htmlmixed/htmlmixed":5,"codemirror/mode/javascript/javascript":6,"file-saver":8}],11:[function(require,module,exports){
"use strict";

function _typeof(obj) { "@babel/helpers - typeof"; if (typeof Symbol === "function" && typeof Symbol.iterator === "symbol") { _typeof = function _typeof(obj) { return typeof obj; }; } else { _typeof = function _typeof(obj) { return obj && typeof Symbol === "function" && obj.constructor === Symbol && obj !== Symbol.prototype ? "symbol" : typeof obj; }; } return _typeof(obj); }

var defaultConfig = require('../../minify-config.json');
...
module.exports = function () {
  ...
  MinifyHelper.setConfig = function (newConfig) {
  ...
  MinifyHelper.load = function () {
    if (!window.Minify) {
      return console.error('html-minifier not loaded.');
    }

    try {
      var lsConfigJSON = window.localStorage.getItem('html-minifier-config');
  ...
  MinifyHelper.minify = function () {
    ...
    var callbackFailSafeProxy = function callbackFailSafeProxy(data) {
      if (data) {
        callback(data);
      } else {
    ...
    var callbackOverride = callback;

    switch (type) {
      case 'js':
        minifyRequest.input = "<script>".concat(input, "</script>");
        minifyRequest.config = Object.assign({}, MinifyHelper.config, {
          minifyJS: config || true
        });

        callbackOverride = function callbackOverride() {
      ...
      case 'css':
        ...
        minifyRequest.input = "<style>".concat(input, "</style>");
        minifyRequest.config = Object.assign({}, MinifyHelper.config, {
          minifyCSS: config || true
        });
    ...
    (MinifyHelper.workerMinify || MinifyHelper.localMinify)(minifyRequest.input, minifyRequest.config, callbackOverride, errorCallback);
  };

  MinifyHelper.minifyJS = function (input, config, callback, errorCallback) {
    MinifyHelper.minify('js', input, config, callback, errorCallback);
  };

  MinifyHelper.minifyCSS = function (input, config, callback, errorCallback) {
    MinifyHelper.minify('css', input, config, callback, errorCallback);
  };

  MinifyHelper.minifyHTML = function (input, config, callback, errorCallback) {
    MinifyHelper.minify('html', input, config, callback, errorCallback);
  };

  MinifyHelper.localMinify = function (input, config, callback, errorCallback) {
    ...
    try {
      var output = window.Minify(input, Object.assign({}, config, {
        log: log
      }));
...
},{"../../minify-config.json":1}],12:[function(require,module,exports){
"use strict";
